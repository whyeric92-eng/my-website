...
export const projects: Project[] = [
    {
        ...
        bullets: [
            'Developed a cross-platform mobile application using React Native (Expo) to create an interactive AI dialogue system, featuring a historical "Qin Shi Huang" persona — search "TPCC-app" in the Apple App Store or Google Play.',
            ...
            "Designed and implemented a distributed rate-limiting system using atomic PostgreSQL upserts (INSERT ... ON CONFLICT DO UPDATE) rather than in-memory counters, ensuring correct quota enforcement across horizontally-scaled, stateless Cloud Run instances under concurrent request load.",
            "Implemented client-side session management with Expo SecureStore, transparently handling both authenticated-user and guest-token lifecycles (persistence, reuse, and silent re-issuance) across app restarts.",
            "Containerized the backend with Docker to standardize local development, then deployed the containerized service to Google Cloud Run with GitHub-integrated continuous deployment, managing environment-specific configuration (secrets, production feature flags) across local and cloud environments.",
        ],
        skills: ["PostgreSQL", "FastAPI", "SQLModel", "Alembic", "React Native", "Figma (Software)", "Docker", "Google Gemini API", "Google Cloud Run"],
    },
    {
